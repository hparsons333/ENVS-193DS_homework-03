...
![](images/Screen Shot 2026-05-26 at 9.24.10 PM.png)

### 4b. Visual clarity

**How clearly does the table represent the data underlying tests? 2-4 sentences.**

I would say that the visual representation of the data is mostly clear, and it's easy to understand what the variables are, how they are organized into three sections (Water, vegetation, and soil), what the p-value is for the Wilcoxon rank sum tests, and the actual observed values for natural and restored pools (with units). However, it may have been more helpful to put the variables with significant differences at the top of each section, so the viewer would instantly direct their attention there.

Additionally, when I discussed this table with An after homework 2, she pointed out that it was strange for them to use the sum of minimum/maximum daily temperatures. I still haven't found a reason for this in the paper, and a better way to address this variable would be to take the average of minimum/maximum daily temperatures rather than the sum. 
...
They provided good spacing in between the titles of variables and columns, as well as between the values of the columns themselves, which was good to reduce clutter. Additionally, they bolded the variables that were found to have a significant difference in value between restored and natural vernal pools. One thing they could've done to decrease visual clutter is to separate the different sections (Water, Vegetation, and Soil) into three different tables, because at first glance all the variables together are a little overwhelming. 
